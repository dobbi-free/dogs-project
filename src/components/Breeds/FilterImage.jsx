...
const FilterImage = ({
  ...
  const sortAtoZ = () => {
    const newArray = state.sort((a, b) => {
      const nameA = a.breeds[0]?.name ? a.breeds[0].name.toLowerCase() : "z",
        nameB = b.breeds[0]?.name ? b.breeds[0].name.toLowerCase() : "z";
  ...
  return (
    <div className={style.filter_block}>
      <select
        value={breedId.id}
        onChange={(e) => {
          const newState = { ...breedId };
          newState.id = e.target.value;
          setBreedId(newState);
        }}
        className={style.filter_breeds}
      >
      ...
      <div className={style.filter_wrapp}>
        <select
          ...
          onChange={(e) => {
            ...
            newState.count = Number(e.target.value);
            setLimit(newState);
          }}
          className={style.filter_limits}
        >
          <option className={style.option} defaultValue={" "} value={5}>
            Limit : 5
          </option>
          <option className={style.option} value={10}>
            Limit : 10
        ...
        <button className={style.button_filter_letter} onClick={sortZtoA}>
          <svg
            width="19"
            height="20"
            viewBox="0 0 19 20"
        ...
        <button className={style.button_filter_letter} onClick={sortAtoZ}>
          <svg
            width="19"
            height="20"
            viewBox="0 0 19 20"
